...
-   Modify existing variables for a date
...
#| label: wd-3-factor
# Lab 4 Question 6
#Modified original code to filter factor (could not find example elsewhere)
...
#Modified the original code  to combine two chunks into a single pipeline, used a join_by instead of using 'by' which was less efficient and tidy.

mfh_ids <- drivers_license |>
  filter(height %in% c(65,66,67),
         gender == 'female',
         hair_color == 'red',
         car_make == 'Tesla',
         str_detect(car_model, " S$")
  ) |>
  left_join(person, join_by(id == license_id))
```
...
#Modified the original code to use the 'join_by' argument instead of using 'by'
#See 'id' tibble (an output of a previous cell) in screenshot below

id_1 <- person |>
  right_join(id, join_by(id == person_id))
```

-   ![](images/clipboard-3310247582.png){width="186"}
-   `inner_join()`
...
#| label: wd-6-anti
# Lab 3 Question 12
# Modified original code to work with an anti join

evals_cleaned_q12 <- evals_cleaned |>
  filter(sex == 'female') |>
  anti_join(
    filter(evals_cleaned, academic_degree %in% c('no_dgr', 'ma'))
  ) |>
  group_by(teacher_id) |>
  summarise(avg_pct_stu = mean(resp_share))
...
Challenge 2 (see .html file in portfolio GitHub repository)

MatchingScores (see .html in GitHub repository: this is an analysis I did in R for a small portion of my thesis in which I leveraged skills from this class)
...
#| label: r-2-3
# Lab 7 Question 4
# Added additional comments for improved documentation

rescale_01 <- function(vec, na.rm = TRUE) {
  #Stop function if the vector does not contain numeric values, or if the vector length is greater than 1
  if (!is.numeric(vec[1])){
  ...
  #Using range to get the range of the function
  ...
  #Removing NA values if the user specifies
  if (na.rm){
...
#| label: r-3-example
# Lab 4 Question 5
#Used slice_max to ensure highest mean_score will always be returned
#Modified code such that everything is performed in a single pipeline
...
NA_outliers <- function(vec, min_value, max_value) {
  stopifnot("Vector must be numeric and min_value must be less than max_value" 
            = is.numeric(vec[1]) & max_value >= min_value)
  vec <- case_when(
    (vec >= min_value & vec <= max_value) ~ vec,
                   .default = NA)
  return(vec)
}
```
...
#| label: dvs-7-1
# Lab 9 question 2
 #Created a heatmap to color code table such that the frequency of the number of babies returned correctly is represented by the shade
...
#| label: dvs-7-2
#Lab 8 Question 2
#Reduced number of decimals, created a striped table for easier readibility
...
evals |>
  slice(1:10) |> #https://www.projectpro.io/recipes/get-top-n-rows-of-dataframe-by-date-r
   knitr::kable(caption = "Data Types of Columns in Surveys Dataset",
                digits = 2,
                align = 'l') |>
   kable_styling(bootstrap_options = c("striped", "hover"))
```

![](images/clipboard-474261060.png)
...
#| label: pe-1-map-1
#Lab 8 question 1
#Rewrote map_chr to have more modern syntax
surveys |>
  map_chr(~ class(.x)) |>
  bind_cols(name = names(surveys)) |>
  rename('Type' = 1) |> #https://sparkbyexamples.com/r-programming/rename-column-by-index-position-in-r/#:~:text=Rename%20Column%20By%20Index%20by,R%2C%20indexing%20starts%20with%201.
  pivot_wider(values_from = Type, names_from = name) |> #https://stackoverflow.com/questions/36396911/r-move-index-column-to-first-column
  knitr::kable(align = 'l',
               caption = "Data Types of Columns in Surveys Dataset") |>
  kable_styling()
```
...
NA_outliers <- function(vec, min_value, max_value) {
  stopifnot("Vector must be numeric and min_value must be less than max_value" = is.numeric(vec[1]) & max_value >= min_value)
  vec <- case_when(
    (vec >= min_value & vec <= max_value) ~ vec,
                   .default = NA)
  return(vec)
...
#Did not have any data frame functions so I modified my vecotr function to take a data frame input.

fish_condition <- function(df, length_col, weight_col) {
  return(
  df |>
    mutate(condition = {{ weight_col}} / ({{length_col}} ^ 3) /  100)
  )
}
```
...
#| label: pe-3-across
# Lab 8 Question 3
# Used across for a single fct_recode call
  ...
  mutate(seniority = factor(
    if_else(
      as.numeric(seniority) <= 4, 
            'Junior (4 years or less)', 
            'Senior (more than 4 years)')),
    across(.cols = c(gender, academic_degree),
           .fns = ~ fct_recode(.f = .x,
                        'Male' = 'male',
                        'Female' = 'female',
                        'Doctorate' = 'dr',
...
#| label: dsm-1-1
#Lab 9 Question 1
#Simulated from numbers 1 through 4 (discrete uniform distribution)

randomBabies <- function(nBabies){
  baby_data <- tibble(parent = 1:nBabies,
                      returned = sample(1:nBabies,
                                        size = nBabies,
                                        replace = TRUE))
  baby_data |>
    filter(returned == 1) |>
